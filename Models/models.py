...
class RB(nn.Module):
    """
    Implements a Residual Block (RB) with Group Normalization and SiLU activation.
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.skip = nn.Identity() if out_channels == in_channels else nn.Conv2d(in_channels, out_channels, kernel_size=1)
...
class FCB(nn.Module):
    """
    Fully Convolutional Backbone (FCB) for downsampling and upsampling feature extraction.

    Args:
        in_channels (int, optional): Number of input channels. Default is 3.
        min_level_channels (int, optional): Number of channels at the smallest level. Default is 32.
        min_channel_mults (list, optional): Multipliers for the number of channels at each level. Default is [1, 1, 2, 2, 4, 4].
        n_levels_down (int, optional): Number of downsampling levels. Default is 6.
        n_levels_up (int, optional): Number of upsampling levels. Default is 6.
        n_RBs (int, optional): Number of residual blocks per level. Default is 2.
        in_resolution (int, optional): Resolution of the input image. Default is 352.
    """
    def __init__(
        self,
        in_channels=3,
...
class TB(nn.Module):
    """
    Transformer Backbone (TB) using the Pyramid Vision Transformer (PVT).
    """
    def __init__(self):
        super().__init__()

        # Initialize the Pyramid Vision Transformer
        ...
        # Spatial Feature Aggregators (SFA)
    ...
    def get_pyramid(self, x):
        """
        Extracts multi-scale feature maps using the PVT backbone.

        Args:
            x (torch.Tensor): Input tensor.

        Returns:
            list: Pyramid of feature maps.
        """
        pyramid = []
        B = x.shape[0]
...
class FCBFormer(nn.Module):
    """
    Combines the Transformer Backbone (TB) and Fully Convolutional Backbone (FCB) for feature extraction.

    Args:
        size (int, optional): Resolution of the input image. Default is 256.
    """
